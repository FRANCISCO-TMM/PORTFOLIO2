...
function renderTasks() {
  ...
  if (tasks.length === 0) {
    taskList.innerHTML = '<li class="task-empty">No tasks logged yet — add your first academic task above.</li>';
  } else {
    tasks.forEach(task => {
      ...
      completeBtn.textContent = task.completed ? 'Reopen' : 'Resolve';
      completeBtn.addEventListener('click', () => toggleTask(task.id));

      const deleteBtn = document.createElement('button');
      deleteBtn.className = 'delete-btn';
      deleteBtn.textContent = 'Strike';
  ...
  if (stats) {
    ...
    stats.innerHTML = `<strong>${completedCount}</strong> of <strong>${tasks.length}</strong> entries resolved`;
  }
}
